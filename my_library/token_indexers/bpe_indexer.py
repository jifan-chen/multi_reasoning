...
import torch
from my_library.pytorch_transformers.tokenization_roberta import RobertaTokenizer
...
_NEVER_LOWERCASE = ['<s>', '</s>', '<pad>', '<mask>', '<unk>']


class BPEIndexer(TokenIndexer[int]):
    """
    A token indexer that does the bpe-tokenization (e.g. for RoBERTa embeddings).
    If you are using one of the pretrained RoBERTa models, you'll want to use the ``PretrainedRoBertaIndexer``
    ...
    """
    SPECIAL_TOKENS = _NEVER_LOWERCASE

    def __init__(self,
                 vocab: Dict[str, int],
                 bpe_tokenizer: Callable[[str], List[str]],
                 namespace: str = "bpe",
        ...
        # In RoBerta tokenizer, _tokenize function will also split special tokens.
        # Although tokenize will not but it is more complex.
        # So we use _tokenize and check whether input is a special tokens beforehand
        ...
        self._start_piece_ids = [bpe_id
                                 for token in (start_tokens or [])
                                 for bpe_id in self.token2bpeids(token)]
        self._end_piece_ids = [bpe_id
                               for token in (end_tokens or [])
                               for bpe_id in self.token2bpeids(token)]

        # Convert the separator_token to wordpiece_ids
        self._separator_ids = [bpe_id
                               for token in separator_tokens
                               for bpe_id in self.token2bpeids(token)]

        self._cache: Dict[str, List[str]] = {}

    def token2bpeids(self, token):
        if token in self.SPECIAL_TOKENS:
            return [self.vocab[token]]
        else:
            '''
            return [self.vocab[bpe] for bpe in self.bpe_tokenizer(token)]
            '''
            token = ' ' + token
            if token in self._cache:
                return self._cache[token]
            if sys.version_info[0] == 2:
                token = ''.join(self.bpe_tokenizer.byte_encoder[ord(b)] for b in token) # Maps all our bytes to unicode strings, avoiding controle tokens of the BPE (spaces in our case)
            else:
                token = ''.join(self.bpe_tokenizer.byte_encoder[b] for b in token.encode('utf-8')) # Maps all our bytes to unicode strings, avoiding controle tokens of the BPE (spaces in our case)
            bpe_tokens = [bpe_token for bpe_token in self.bpe_tokenizer.bpe(token).split(' ')]
            bpe_ids = [self.vocab[bpe] for bpe in bpe_tokens]
            self._cache[token] = bpe_ids
            return bpe_ids
    ...
    def tokens_to_indices(self,
        ...
        token_bpe_ids = [self.token2bpeids(token) for token in text]
        ...
        flat_bpe_ids = [bpe_id for token in token_bpe_ids for bpe_id in token]
        ...
        flat_token_type_ids = _get_token_type_ids(flat_bpe_ids, self._separator_ids)
        ...
        if len(flat_bpe_ids) <= window_length:
            ...
            bpe_windows = [self._add_start_and_end(flat_bpe_ids[i:i + window_length])
                                 for i in range(0, len(flat_bpe_ids), stride)]
            ...
            last_window = bpe_windows[-1][1:]
            penultimate_window = bpe_windows[-2]
            if last_window == penultimate_window[-len(last_window):]:
                bpe_windows = bpe_windows[:-1]
        ...
        bpe_ids = [bpe_id for sequence in bpe_windows for bpe_id in sequence]
        ...
        return {index_name: bpe_ids,
                f"{index_name}-offsets": offsets,
                f"{index_name}-type-ids": token_type_ids,
                "mask": mask}

    def _add_start_and_end(self, bpe_ids: List[int]) -> List[int]:
        return self._start_piece_ids + bpe_ids + self._end_piece_ids
...
class PretrainedRoBertaIndexer(BPEIndexer):
    # pylint: disable=line-too-long
    """
    A ``TokenIndexer`` corresponding to a pretrained RoBERTa model.
    Parameters
    ----------
    ...
    def __init__(self,
        ...
        super().__init__(vocab=roberta_tokenizer.encoder,
                         #bpe_tokenizer=roberta_tokenizer._tokenize,
                         bpe_tokenizer=roberta_tokenizer,
                         namespace="roberta",
