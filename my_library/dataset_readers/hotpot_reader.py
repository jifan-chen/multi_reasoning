...
def get_dep_mask(dep_heads):
    dep_mask = np.zeros((len(dep_heads), len(dep_heads)))
    valid_heads = [h for h in dep_heads if not h == -1] # filter out the token whose parent is root
    valid_childs = [i for i, h in enumerate(dep_heads) if not h == -1]
...
def make_reading_comprehension_instance(question_tokens: List[Token],
                                        ...
                                        sent_dep_heads: List[List[int]] = None,
    ...
    sent_dep_heads : ``List[List[int]]``, optional
        The dependency parents for each token for each sentence in the passage, zero-indexing.
    ...
    sent_dep_masks: List[Field] = []
    if token_spans_sent:
        for (start, end), dep_heads in zip(token_spans_sent, sent_dep_heads):
            if start < para_limit and end < para_limit:
                sent_spans.append(SpanField(start, end, passage_field))
                sent_dep_masks.append(ArrayField(get_dep_mask(dep_heads)))
    fields['sentence_spans'] = ListField(sent_spans)
    fields['sentence_dep_masks'] = ListField(sent_dep_masks)
...
class HotpotDatasetReader(DatasetReader):
    ...
    def text_to_instance(self,  # type: ignore
                         ...
                         passage_offsets: List[Tuple] = None,
                         sent_dep_heads: List[List[int]] = None) -> Instance:
