...
def make_reading_comprehension_instance(question_tokens: List[Token],
    ...
    if answer_texts[0] == 'yes':
        ...
        fields['span_start'] = SequenceLabelField([0]*len(passage_tokens), passage_field)
        fields['span_end'] = SequenceLabelField([0]*len(passage_tokens), passage_field)
        ...
        if token_spans:
            ...
            labels_start = [0]*len(passage_tokens)
            labels_end = [0] * len(passage_tokens)
            for span_start, span_end in token_spans:
                if span_start < para_limit and span_end < para_limit:
                    labels_start[span_start] = 1
                    labels_end[span_end] = 1
            fields['span_start'] = SequenceLabelField(labels_start, passage_field)
            fields['span_end'] = SequenceLabelField(labels_end, passage_field)
        else:
            fields['span_start'] = SequenceLabelField([0]*len(passage_tokens), passage_field)
            fields['span_end'] = SequenceLabelField([0]*len(passage_tokens), passage_field)
...
@DatasetReader.register("hotpot_reader_all_span")
class HotpotAllSpanDatasetReader(DatasetReader):
    """
