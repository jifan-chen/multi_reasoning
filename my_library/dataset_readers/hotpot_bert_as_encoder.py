...
@DatasetReader.register("hotpot_bert_as_encoder")
class HotpotDatasetReader(DatasetReader):
    ...
    def process_raw_instance(self, article):
        ...
        for chain in pred_chains[:top_k]:
            for e in chain:
                if e not in combined_chains_id and len(combined_chains_id) < num_sp:
        ...
        for sent_id in combined_chains_id:
            ...
            sent_offset_qp = [(tk.idx + len(concat_article),
                               tk.idx + len(tk.text) + len(concat_article)) for tk in tokenized_sent]

            passage_offsets.extend(sent_offset_qp)
            concat_article += sent
        ...
        sp_starts = [self.find_span_starts(s, concat_article) for s in combined_chains]
    ...
    def text_to_instance(self,  # type: ignore
        ...
        for char_span_start, char_span_end in char_spans:
            ...
            # print(passage_tokens[span_start:span_end+1])
            if error:
