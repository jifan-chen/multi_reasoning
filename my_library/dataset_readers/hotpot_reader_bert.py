...
def make_reading_comprehension_instance(question_tokens: List[Token],
    ...
    input(fields["evd_chain_labels"])
    return Instance(fields)


@DatasetReader.register("hotpot_reader_bert")
        ...
        for para in paragraphs:
            ...
            for sent_id, sent in enumerate(cur_para):
                ...
                if len(tokenized_sent) > 0:
                    ...
                    sent_offset = [(tk.idx + len(concat_para),
                                    tk.idx + len(tk.text) + len(concat_para)) for tk in tokenized_sent]
