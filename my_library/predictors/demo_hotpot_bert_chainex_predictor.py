...
def get_token_spans_sent(passage_sent_tokens):
    passage_tokens = []
    token_spans_sent = []
    for sent_tokens in passage_sent_tokens:
        s = len(passage_tokens)
        e = s + len(sent_tokens) - 1
        token_spans_sent.append([s, e])
        passage_tokens += sent_tokens
...
class DemoHotpotPredictor(Predictor):
	...
	# coref res
        ...
        return {"doc":              passage_tokens,
                "attns":            heads_doc_res,
                "qc_scores":        None,#qc_scores,
                "qc_scores_sp":     None,#qc_scores_sp,
