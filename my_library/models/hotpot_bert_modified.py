...
@Model.register("hotpot_bert_modified")
class BidirectionalAttentionFlow(Model):
    ...
    def forward(self,  # type: ignore
        ...
        question_bert = self._text_field_embedder(question)
        passage_bert = self._text_field_embedder(passage)
        ques_mask = util.get_text_field_mask(question).float()
        context_mask = util.get_text_field_mask(passage).float()

        batch_size = ques_mask.size()[0]
        ques_output = self._dropout(self._phrase_layer(question_bert, ques_mask))
        context_output = self._dropout(self._phrase_layer(passage_bert, context_mask))
