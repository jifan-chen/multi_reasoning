...
@Model.register("hotpot_bert_gate_wo_ans_para")
...
@Seq2SeqEncoder.register("indep_bert_span_gate_para")
class SpanGate(Seq2SeqEncoder):
    ...
    def forward(self,
        ...
        batch_size, num_spans, max_batch_span_width = spans_mask.size()
        # Shape: (batch_size*num_spans, max_span_length, embedding_dim)
        max_pooled_span_emb = torch.max(spans_tensor, dim=1)[0]
        print('max_pooled:', max_pooled_span_emb.shape)
        # shape: (batch_size, num_spans)
