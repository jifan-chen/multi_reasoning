"""
A ``TokenEmbedder`` which uses one of the RoBERTa models
...
from my_library.pytorch_transformers import RobertaModel
...
class PretrainedRoBertaModel:
    ...
    """
    _cache: Dict[str, RobertaModel] = {}

    @classmethod
    def load(cls, model_name: str, cache_model: bool = True) -> RobertaModel:
        if model_name in cls._cache:
            return PretrainedRoBertaModel._cache[model_name]
...
class RoBertaEmbedder(TokenEmbedder):
    ...
    num_start_tokens : int, optional (default: 1)
        The number of starting special tokens input to BERT (usually 1, i.e., <s>)
    num_end_tokens : int, optional (default: 1)
        The number of ending tokens input to BERT (usually 1, i.e., </s>)
    ...
    def __init__(self,
        ...
        self.output_dim = roberta_model.config.hidden_size
        ...
        if not top_layer_only:
            self._scalar_mix = ScalarMix(roberta_model.config.num_hidden_layers,
                                         do_layer_norm=False,
    ...
    def forward(self,
        ...
        if needs_split:
            ...
            # shape: (num_window*batch_size, d1, ..., dn, max_pieces)
        ...
        # pooled_output shape: (num_window*batch_size*d1*...*dn, hidden_size)
        # all_encoder_layers shape: (1+num_layer) * (num_window*batch_size*d1*...*dn, max_pieces, hidden_size)
        _, pooled_output, all_encoder_layers = self.roberta_model(input_ids=util.combine_initial_dims(input_ids),
                                                                  token_type_ids=util.combine_initial_dims(token_type_ids),
                                                                  attention_mask=util.combine_initial_dims(input_mask))
        # shape: (num_layer, num_window*batch_size*d1*...*dn, max_pieces, hidden_size)
        all_encoder_layers = torch.stack(all_encoder_layers[1:])

        if needs_split:
            # First, unpack the output embeddings into one long sequence again
            # shape: num_window * (num_layer, batch_size*d1*...*dn, max_pieces, hidden_size)
            unpacked_embeddings = torch.split(all_encoder_layers, int(np.prod(initial_dims)), dim=1)
            # shape: (num_layer, batch_size*d1*...*dn, num_window*max_pieces, hidden_size)
...
@TokenEmbedder.register("roberta-pretrained")
class PretrainedRoBertaEmbedder(RoBertaEmbedder):
    ...
    def __init__(self, pretrained_model: str, requires_grad: bool = False, top_layer_only: bool = False,
                 scalar_mix_parameters: List[float] = None, max_pieces: int = 512, output_cls: bool = False) -> None:
        model = PretrainedRoBertaModel.load(pretrained_model)

        for param in model.parameters():
            param.requires_grad = requires_grad

        super().__init__(roberta_model=model, top_layer_only=top_layer_only, scalar_mix_parameters=scalar_mix_parameters,
                         max_pieces=max_pieces, output_cls=output_cls)
